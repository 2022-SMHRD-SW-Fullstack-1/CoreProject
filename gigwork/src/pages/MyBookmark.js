        ...
        <div className='contentPart'>
          ...
          <tr>
            ...
            {postcontent.map((item, idx) =>
              <tr>
                <td ><input type='checkbox' post_num={item.POST_NUM} onChange={handleAllcheck} value={checkItems} id='trpart'></input></td>
                <td ><span key={item + idx} id='trpart'>{item.POST_NUM}</span></td>
                <td ><span id='trpart'>{item.WORKTIME_S.replace('T', ' ').replace(/\..*/, '').substring(0, 16)}</span></td>
                <td ><span name={item.POST_NUM} onClick={goToDetail} id='trpart'>{item.TITLE}</span><br /></td>
                <td ><span id='trpart'>{item.POST_PAY}</span></td>
              </tr>)}
          </table>
